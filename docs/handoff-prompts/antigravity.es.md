...
La solución es ejecutar el binario UNA VEZ en una terminal real (TTY presente), completar device flow, y dejar el token cacheado en disco. Después Antigravity arranca, el MCP encuentra el token cacheado y todas las tool calls funcionan sin re-auth hasta que expire (~12 horas, con refresh_token que extiende hasta el límite del IdP — típicamente 30 días).
